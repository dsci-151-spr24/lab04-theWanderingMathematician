...
1.  What are the dimensions of the Denny's dataset?
    ...
    - *The Denny's dataset is `r nrow(dennys)` rows by `r ncol(dennys)` columns. Each row is gives data on a single Denny's location somewhere in the USA. The variables are `adress`, `city`, `state`, `zip`, `longitude`, and `latitude`.*

2.  What are the dimensions of the La Quinta's dataset?
    What does each row in the dataset represent?
    What are the variables?
    - *The La Quinta dataset is `r nrow(laquinta)` rows by `r ncol(laquinta)` columns. Each row gives data on a single La Quinta Inn somewhere in the USA. The variables are `adress`, `city`, `state`, `zip`, `longitude`, and `latitude`.*
